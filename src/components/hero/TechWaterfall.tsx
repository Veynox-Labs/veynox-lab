...
import { Code2, Server, Database, Cloud, Terminal, MonitorSmartphone, Cpu, Box, Layout, Sparkles } from "lucide-react";

const techIcons = [Code2, Server, Database, Cloud, Terminal, MonitorSmartphone, Cpu, Box, Layout, Sparkles];
const techNames = ["React", "Next.js", "TypeScript", "JavaScript", "Node.js", "Tailwind CSS", "AWS", "Figma", "GitHub", "Vercel"];
...
export default function TechWaterfall() {
  ...
  return (
    <div className="absolute top-24 bottom-0 left-0 right-0 overflow-hidden pointer-events-none z-0">
      ...
      {/* Top and Bottom Fade */}
      <div className="absolute top-0 left-0 w-full h-40 bg-gradient-to-b from-bg-main via-bg-main/80 to-transparent z-10"></div>
      <div className="absolute bottom-0 left-0 w-full h-40 bg-gradient-to-t from-bg-main via-bg-main/80 to-transparent z-10"></div>
        ...
        return (
          <motion.div
            ...
            transition={{
              ...
              x: { repeat: Infinity, duration: item.duration * 0.6, ease: "easeInOut" }
            }}
            className="absolute top-0 flex flex-col items-center justify-center gap-2"
            style={{ left: item.left, scale: item.size, opacity: item.opacity }}
          >
            {/* The Glassmorphic Pill */}
            <div className="relative group">
              {/* Animated Glow Behind Pill */}
              <div className={`absolute inset-0 bg-gradient-to-r ${gradientBorders[item.gradientIndex]} opacity-20 blur-xl rounded-full`}></div>
              ...
              <div className="relative px-5 py-2.5 rounded-full bg-white/5 backdrop-blur-xl border border-white/10 shadow-2xl flex items-center gap-3 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/5 to-white/0 transform translate-x-[-100%] animate-[shimmer_3s_infinite]"></div>
                
                <Icon className="text-brand-electric w-4 h-4 md:w-5 md:h-5" />
                <span className="text-white font-semibold text-sm tracking-wider">{item.name}</span>
              </div>
            </div>
          </motion.div>
        );
